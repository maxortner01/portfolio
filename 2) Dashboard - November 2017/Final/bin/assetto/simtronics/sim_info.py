...
def demo():
    return {'Speed': info.physics.speedKmh, 'rpm': info.physics.rpms, 'maxRpm': info.static.maxRpm, 'tyreWear': list(info.physics.tyreWear),
		   'Gear': info.physics.gear, 'Throttle': info.physics.gas, 'Brake': info.physics.brake, 'maxFuel': info.static.maxFuel,
		   'fuel': info.physics.fuel, 'pitch': info.physics.pitch, 'roll': info.physics.roll, 'abs': info.physics.abs,
           'tc': info.physics.tc, 'auto': info.physics.autoShifterOn, 'damage': info.physics.carDamage, 'status': info.graphics.status,
           'wheel-pressure': info.physics.wheelsPressure}
        #time.sleep(0.1)
